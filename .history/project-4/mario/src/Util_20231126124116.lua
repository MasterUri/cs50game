...
function GenerateQuadsKeys(atlas)
    ...
    for i = 0, 3 do
        quads[i + 1] = love.graphics.newQuad(x + (i * 16), y, 16, 16, atlas:getDimensions())
    end
    return quads
end
...
function GenerateQuadsFlagPoles(atlas)
    ...
    for i = 0, 5 do
        quads[i + 1] = love.graphics.newQuad(x + (i * 16), y, 16, 48, atlas:getDimensions())
    end
...
function GenerateQuadsFlags(atlas)
    ...
    for i = 0, 3 do
        for j = 0, 2 do
            quads[counter] = love.graphics.newQuad(x + (j * 16), y, 16, 16, atlas:getDimensions())
            counter = counter + 1
        end
        y = y + 16
    end
