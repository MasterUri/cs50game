...
function PlayState:checkForGround()
    local mapTiles = self.tileMap
    local horizontalSpawnPoint = 0

    for i = 1, #mapTiles do
        for j = 1, #mapTiles[i] do
            if mapTiles[i][j].id == TILE_ID_GROUND then
                horizontalSpawnPoint = mapTiles[i][j].x * TILE_SIZE
                break
            end
        end
    end
    self.test = horizontalSpawnPoint
...
function PlayState:render()
    ...
    love.graphics.printf(self.test, 1, VIRTUAL_HEIGHT / 2 - 40 + 1, VIRTUAL_WIDTH, 'right')
end
