                ...
                if x >= 85 and x <= 90 and lockPlaced == false then
                    table.insert(objects,
                        GameObject {
                            ...
                            onUnlock = function (player, object)
                                gSounds['pickup']:play()
                                player.hasKey = false
    
                                local flag = GameObject {
                                    texture = 'flags',
                                    x = (width - 3) * TILE_SIZE + 10,
                                    y = (6 - 1) * TILE_SIZE,
                                    width = 16,
                                    height = 16,
                                    frame = FLAGS[keyColor][4],
                                    collidable = true,
                                }
                                ...
                                local flagPole = GameObject {
                                    ...
                                    onRaise = function (object)
                                        ...
                                        Timer.every(0.5, function () 
                                            flag.frame = FLAGS[keyColor][flagId]
                                            flagId = flagId + x
                                            x = -x
                                        end)
                                        
                                        Timer.tween(0.5, {
                                            [flag] = {y = (5 * TILE_SIZE) - 32}
                                        })
                                        
                                        if not object.raised then
