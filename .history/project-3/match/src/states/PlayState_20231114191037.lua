    ...
    for y = 1, 8 do
        for x = 1, 8 do 
            if self.board.tiles[y][x].shiny then
                trable.insert(self.isShiny, self.board.tiles[y][x])
            else
                table.remove(self.isShiny, self.board.tiles[y][x])
            end
        end
    end

    -- making the tile shine
    if self.shinyTimer == 0 then
        Timer.tween(1, {
            [self] = {shinyTimer = 96}
        })
    elseif self.shinyTimer == 96 then
        Timer.tween(1, {
            [self] = {shinyTimer = 0}
        })
    end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            -- add 1 second to timer for each tile in a match
        ...
        if #self.board.removeShiny > 0 then
            for y = 1, 8 do
                for k, coord in pairs(self.board.removeShiny[1]) do
                    if y == coord then
                        for x = 1, 8 do
                            for j, coord in pairs(self.board.removeShiny[2]) do
                                if x == coord then
                                    self.board.tiles[y][x].shiny = false
                                end
                            end
                        end
                    end
