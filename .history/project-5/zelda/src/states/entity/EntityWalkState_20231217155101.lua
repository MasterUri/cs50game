...
function EntityWalkState:update(dt)
    ...
    -- checking if entity collided with a pot
    if self.dungeon ~= nil then
        local roomObjects = self.dungeon.currentRoom.objects
        local collided = false
        for k, object in pairs(roomObjects) do
            if object.type == 'pot' and self.entity:collides(object) then
                if self.entity.direction == 'left' and collided == false then
                    self.entity.x = self.entity.x - self.entity.walkSpeed * dt
                    collided = true
                    
                    if self.entity.x <= object.x + TILE_SIZE then
                        self.entity.x = object.x + TILE_SIZE
                        self.bumped = true
                    end
                elseif self.entity.direction == 'right' and collided == false then
                    self.entity.x = self.entity.x + self.entity.walkSpeed * dt
                    collided = true
            
                    if self.entity.x + self.entity.width >= object.x then
                        self.entity.x = object.x - self.entity.width
                        self.bumped = true
                    end
                elseif self.entity.direction == 'up' and collided == false then
                    self.entity.y = self.entity.y - self.entity.walkSpeed * dt
                    collided = true
            
                    if self.entity.y <= object.y + TILE_SIZE - self.entity.height / 2 then
                        self.entity.y = object.y + TILE_SIZE - self.entity.height / 2
                        self.bumped = true
                    end
                elseif self.entity.direction == 'down' and collided == false then
                    self.entity.y = self.entity.y + self.entity.walkSpeed * dt
                    collided = true

                    if self.entity.y + self.entity.width >= object.y then
                        self.entity.y = object.y - self.entity.width
                        self.bumped = true
                    end
                end
            else
