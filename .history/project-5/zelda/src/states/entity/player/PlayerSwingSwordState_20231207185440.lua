...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if entity:collides(self.swordHitbox) then
            ...
            local heartX = entity.x
            local heartY = entity.y
            if not entity.dead and entity:collides(self.player) then
                if entity.x > (VIRTUAL_WIDTH / 2) then
                --     if (entity.x - self.player.x) < self.player.x then
                --         heartX = heartX - (entity.width + self.player.width)
                --     else
                        heartX = heartX - entity.width
                --     end
                else
                --     if (entity.x + self.player.x) > self.player.x then
                --         heartX = heartX + (entity.width + self.player.width)
