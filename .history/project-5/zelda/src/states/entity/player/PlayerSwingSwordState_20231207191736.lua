...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if entity:collides(self.swordHitbox) then
            ...
            self.heartX = entity.x
            self.heartY = entity.y
            if not entity.dead and entity:collides(self.player) then
                if entity.x > (VIRTUAL_WIDTH / 2) then
                    -- if (entity.x - self.player.x) < self.player.width then
                    --     self.heartX = self.heartX - (entity.width * 2)
                    -- else
                        self.heartX = self.heartX - entity.width
                    -- end
                else
                    -- if (self.player.x - entity.x) < self.player.width then
                    --     self.heartX = self.heartX + (entity.width * 2)
                    -- else
                        self.heartX = self.heartX + entity.width
                    -- end
            ...
            if entity.dead and not self.player.collisionProcessing then
                local heart = GameObject(
                    GAME_OBJECT_DEFS['heart'],
                    self.heartX,
                    self.heartY
                )
                --self.player.collisionProcessing = false
