...
function EntityWalkState:update(dt)
    ...
    -- position before colision
    local preColX = self.entity.x
    local preColY = self.entity.y
    ...
    -- checking if entity collided with a solid object
    if self.dungeon ~= nil then
        ...
        for k, object in pairs(roomObjects) do
            if object.solid and self.entity:collides(object) then
                self.bump = true
                colObj = object
                self.entity.x = preColX
                self.entity.y = preColY
            end
        end
    end
    self.entity.collidedObject = colObj
end
