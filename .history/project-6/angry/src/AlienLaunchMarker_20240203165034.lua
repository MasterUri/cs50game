    ...
    if not self.launched then
        ...
        if self.launched then
            if love.keyboard.wasPressed('space') then
                self.alien2 = Alien(self.world, 'round', self.shiftedX + 5, self.shiftedY + 5, 'Player')
                self.alien2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)
