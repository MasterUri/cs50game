...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.hit then
            ...
            local posX = self:getAlienX()
            local posY = self:getAlienY()
            
            for i = 2, 3 do
                self.aliens[i] = Alien(self.world, 'round', posX, posY, 'Player')
                
                self.aliens[i].body:setLinearVelocity( 10, 10)
...
function AlienLaunchMarker:getAlienX()
    return self.aliens[1].body.x
end

function AlienLaunchMarker:getAlienY()
    return self.aliens[1].body.y
end
